...
if [[ ! -f "$ENV_FILE" ]]; then
    fail "Missing $ENV_FILE. Copy .env.local.example to .env.local and configure the native database."
fi
...
# All development services use the native PostgreSQL and Redis instances.
...
if [[ "$DATABASE_URL" == *"@postgres:"* ]]; then
    fail "DATABASE_URL must use 127.0.0.1 for local development."
fi

"$PYTHON_BIN" -c 'import psycopg2, os; conn=psycopg2.connect(os.environ["DATABASE_URL"]); conn.close()' \
    || fail "Cannot connect to the native PostgreSQL database configured by DATABASE_URL."
