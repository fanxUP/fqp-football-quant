...
## 运行方式：本机开发，Docker Desktop 运行

日常修改始终在本机工作目录中完成；提交并推送 GitHub 后，再通过 Docker Desktop 运行完整服务栈。源码以 Git 提交为准，容器仅是可重建运行环境；`data/` 中的数据库和备份不提交 Git。
...
部署到 Docker Desktop（自动检查工作区、推送 GitHub、重建容器并做健康检查）：
