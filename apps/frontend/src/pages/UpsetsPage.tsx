...
import UpsetLeagueAccordion from '../features/upsets/UpsetLeagueAccordion';
import { UpsetMetrics } from '../features/upsets/UpsetOverview';
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [summaryResponse, listResponse, reportResponse] = await Promise.all([
        ...
        api.upsets.list({ ...filters, limit: 200, offset: 0 }),
      ...
      {loading ? <LoadingSpinner text="加载冷门研究数据..." size="lg" /> : error ? (
        ...
        <>
          ...
          {items.length === 0 ? <EmptyState icon="🧊" title="没有符合条件的冷门" description="调整日期、等级或玩法筛选" /> : (
            <UpsetLeagueAccordion items={items} onOpen={(eventId) => void openDetail(eventId)} />
          )}
        </>
      )}
