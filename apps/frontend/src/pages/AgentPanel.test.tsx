...
function mockFetch() {
  const fetchMock = vi.fn(async (input: RequestInfo | URL) => {
    ...
    if (url.includes('/api/agent-summary')) {
      return { json: async () => ({ summary: { active_agents: 11, open_tasks: 2, running_jobs: 1, failed_jobs_24h: 0, pending_review_gates: 1, stale_jobs: 0, scheduler_running: true } }) } as Response;
    }
