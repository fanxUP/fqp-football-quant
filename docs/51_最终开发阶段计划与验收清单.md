...
使用本机启动脚本完成开发、发布和长期运行，以 Git 提交 SHA 作为代码版本边界。
...
本项目以 `54_本机运行与DockerDesktop弃用说明.md` 为准：Codex 在本机开发；前端、后端、Scheduler、Worker、PostgreSQL 与 Redis 全部在本机运行。长期任务只在受管的唯一套 Scheduler/Worker 中执行。
