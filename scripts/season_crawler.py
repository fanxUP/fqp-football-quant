...
def crawl_league_full(league_name: str, league_id: int) -> dict[str, Any]:
    """Fetch FULL season schedule into the third-party supplemental table.

    Existing Sporttery matches (matched by league+teams+kickoff) are updated
    with results only. New third-party matches are isolated in
    ``supplemental_matches`` and never inserted into ``official_matches``.
    """
    t0 = time.monotonic()
    ...
    with get_db() as conn:
        with conn.cursor() as cur:
            for m in all_matches:
                ...
                if existing:
                    ...
                    # Store new third-party match separately. Sporttery is the
                    # only source allowed to create official_matches rows.
                    match_status = "Settled" if m["ft_home_goals"] is not None else "Scheduled"
                    cur.execute(
                        """
                        INSERT INTO supplemental_matches
                            (source_name, source_match_id, league_name,
                             home_team_name, away_team_name, kickoff_time,
                             round_name, match_status, full_home_goals,
                             full_away_goals, half_home_goals, half_away_goals,
                             source_url, raw_json, updated_at)
                        VALUES ('500.com', %s, %s, %s, %s, %s, %s, %s,
                                %s, %s, %s, %s, %s, %s, now())
                        ...
                        """,
                        (
                            m["match_id_500"], league_name,
                            m["home_team"], m["away_team"], kt,
                            m["round"], match_status,
                            m["ft_home_goals"], m["ft_away_goals"],
                            m["ht_home_goals"], m["ht_away_goals"],
                            m["source_url"],
                            json.dumps({
        ...
        # Keep the supplemental schedule relationally attached to the
        # configured season after every refresh. This is deliberately scoped
        # to the matching league and 2026 season; it never touches
        # official_matches.
        with conn.cursor() as cur:
            cur.execute(
                """
                UPDATE supplemental_matches sm
                SET competition_season_id = cs.id, updated_at = now()
                FROM competition_seasons cs
                JOIN competitions c ON c.id = cs.competition_id
                JOIN seasons s ON s.id = cs.season_id
                WHERE sm.source_name = '500.com'
                  AND sm.league_name = %s
                  AND c.competition_name_cn = sm.league_name
                  AND s.season_code = '2026'
                  AND (sm.competition_season_id IS DISTINCT FROM cs.id)
                """,
                (league_name,),
            )
            cur.execute(
