...
# For 'Mx1': n == k, one combination of all matches.

PASS_TYPE_REGISTRY: dict[str, list[tuple[int, int]]] = {
    # M串1: all matches in ONE combination (must all be correct)
...
def get_available_pass_types(match_count: int) -> list[str]:
    ...
    Filters PASS_TYPE_REGISTRY to only return types where the M matches
    the current match_count. Also includes 'single' for any match_count >= 1.
    """
    available: list[str] = []
    if match_count >= 1:
...
def calculate_bet_combinations(
    ...
    - 'Mx1': one combo of all M matches
    ...
    if pass_type.endswith("x1") and pass_type != "single":
        # M串1: one combo of all M matches
        n = int(pass_type.split("x")[0])
        if n != n_matches:
            raise ValueError(
                f"Pass type '{pass_type}' requires exactly {n} matches, got {n_matches}"
            )
        return [list(matches)]
